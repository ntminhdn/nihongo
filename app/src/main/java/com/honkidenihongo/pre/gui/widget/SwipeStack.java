...
import com.honkidenihongo.pre.gui.listener.SwipeHelper;
...
public class SwipeStack extends ViewGroup {
    ...
    // Add by Binh.dt.
    private ViewTopSwipeCallback mViewTopSwipeCallback;
    ...
    /**
     * Register a callback to invoke when top view be clicked
     *
     * @param listener The callback that will run
     */
    ...
    /**
     * Add Binh.dt.
     * Register a callback to invoke when top view start swipe.
     *
     * @param listener The callback that will run.
     */
    public void setViewTopSwipeListener(ViewTopSwipeCallback listener) {
        mViewTopSwipeCallback = listener;
    }
    ...
    /**
     * Programmatically top view click
     */
    public void topViewClick() {
        if (getChildCount() == 0) return;
        if (mChildClickListener != null) {
            mChildClickListener.onClick(mTopView);
        }
    }

    /**
     * Add by Binh.dt.
     * View top đang swipe qua phải.
     */
    public void viewTopOnSwipeRight() {
        if (getChildCount() == 0) {
            return;
        }

        if (mViewTopSwipeCallback != null) {
            mViewTopSwipeCallback.onViewTopSwipeRight();
        }
    }

    /**
     * Add by Binh.dt.
     * View top đang swipe qua trái.
     */
    public void viewTopOnSwipeLeft() {
        if (getChildCount() == 0) {
            return;
        }

        if (mViewTopSwipeCallback != null) {
            mViewTopSwipeCallback.onViewTopSwipeLeft();
        }
    }
    ...
    /**
     * Add by Binh.dt.
     * Tạo interface lắng nghe sự kiện ViewTop đang swipe qua trái hay qua phải.
     */
    public interface ViewTopSwipeCallback {
        void onViewTopSwipeRight();

        void onViewTopSwipeLeft();
    }
