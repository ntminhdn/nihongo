...
public class ProgressPieView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            backgroundColor = res.getColor(R.color.default_background_color);
            progressColor = res.getColor(R.color.default_progress_color);
            strokeColor = res.getColor(R.color.default_stroke_color);
            textColor = res.getColor(R.color.default_text_color);
        } else {
            backgroundColor = context.getColor(R.color.default_background_color);
            progressColor = context.getColor(R.color.default_progress_color);
            strokeColor = context.getColor(R.color.default_stroke_color);
...
//        mViewSize = Math.min(width, height);
        mViewSize = Math.max(width, height);
    ...
    public void setMax(int max) {
        if (max <= 0 || max < mProgress) {
            throw new IllegalArgumentException(
                    String.format(Locale.getDefault(), "Max (%d) must be > 0 and >= %d", max, mProgress));
        }
        mMax = max;
        invalidate();
    }
    ...
    public void animateProgressFill(int animateTo) {
        ...
        if (animateTo > mMax || animateTo < 0) {
            throw new IllegalArgumentException(
                    String.format(Locale.getDefault(), "Animation progress (%d) is greater than the max progress (%d) or lower than 0 ", animateTo, mMax));
        }
    ...
    public void setProgress(int progress) {
        if (progress > mMax || progress < 0) {
            throw new IllegalArgumentException(
                    String.format(Locale.getDefault(), "Progress (%d) must be between %d and %d", progress, 0, mMax));
        }
        mProgress = progress;
        if (null != mListener) {
            if (mProgress == mMax) {
     ...
     * @see com.honkidenihongo.pre.gui.widget.ProgressPieView.OnProgressListener
     */
