...
import com.honkidenihongo.pre.common.util.AnimationUtils;
...
public class SwipeHelper implements View.OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (event.findPointerIndex(mPointerId) < 0 || !mListenForTouchEvents || !mSwipeStack.isEnabled()) {
                    return false;
                }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // Add by Binh.dt lắng nghe sự kiện view top swipe qua trái hay phải if khoảng cách lớn hơn >= 0 thì view top swipe qua phải ngược lại qua trái.
                if (dragDistanceX > 0) {
                    mSwipeStack.viewTopOnSwipeRight();
                } else if (dragDistanceX < 0) { // Bắt trường hợp user chạm vào mà ko move cho sự kiện view child click sẽ xảy ra.
                    mSwipeStack.viewTopOnSwipeLeft();
                }
    ...
    private void checkViewPosition() {
        ...
        if (viewCenterHorizontal < parentFirstThird &&
            ...
            if (mObservedView.getX() >= mInitialX - 10 && mObservedView.getX() <= mInitialX + 10
                    && mObservedView.getY() >= mInitialY - 10 && mObservedView.getY() <= mInitialY + 10) {
                resetViewPosition(50);
                mSwipeStack.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        mSwipeStack.topViewClick();
                    }
                }, 50);
            } else {
                resetViewPosition();
    ...
    private void resetViewPosition(long duration) {
        mObservedView.animate()
                .x(mInitialX)
                .y(mInitialY)
                .rotation(0)
